...
1. If `.codex/cse-policy-harness.json` is missing in the current project, use `cost-soma-activate-project` first.
2. Use the `cost-soma-policy-orchestrator` skill.
3. Spawn `cse-policy-evidence` and `cse-policy-application` subagents in parallel.
...
6. Spawn `cse-policy-reviewer` with the original question and draft answer.
