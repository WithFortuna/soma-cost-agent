...
1. Use the `cost-soma-policy-orchestrator` skill.
2. Spawn `policy-evidence` and `policy-application` subagents in parallel.
3. Have both subagents use `scripts/policy_tool.py` through local scripts.
4. Draft the answer in Korean with `결론`, `신청방법`, `글쓰기 포맷`, `유의사항`, and `근거`.
5. Spawn `policy-reviewer` with the original question and draft answer.
6. Revise the final answer from reviewer findings.
7. If the user asked for an actual submit-ready form file, run `form-packet`, ask for any `missing_required_fields`, follow template `guidelines`, then use Documents for `.docx` or Spreadsheets for `.xlsx` generation and QA. Report any `remaining_submission_requirements` such as signatures, evidence images, receipt images, or ID/bank copies. Do not upload externally.
